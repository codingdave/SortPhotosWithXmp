...
public class DateTimeResolver
{
    ...
    public DateTimeResolver(ILogger staticLogger) => _logger = staticLogger;
    ...
    private static DateTime? DateTimeFromIptc(ILogger logger, IReadOnlyList<MetadataExtractor.Directory> directories)
    {
        ...
        if (typedDirectories != null)
        {
            foreach (var directory in typedDirectories)
            {
                if (directory.GetDateCreated() is DateTimeOffset dateCreatedDto)
                {
                    exifDates.Add(1, dateCreatedDto.DateTime);
                }

                if (directory.GetDigitalDateCreated() is DateTimeOffset digitalDateCreatedDto)
                {
                    exifDates.Add(2, digitalDateCreatedDto.DateTime);
                }
            }
        }
